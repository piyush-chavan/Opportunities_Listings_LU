...
function CoursesListing() {
  ...
  const loadInitialData = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('https://googlesheetdata-backend.onrender.com/sheet?spreadsheetId=1uXPkjZVug-va0F67YL035GATEsoLu-8X5QTFxTK_5rA&sheetName=Courses Database');
      if (!response.ok) {
      ...
      console.error(err);
      setError(`Could not load courses data from backend: ${err.message}`);
      setOpportunities([]);
